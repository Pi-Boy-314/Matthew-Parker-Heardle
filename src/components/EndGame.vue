...
setInterval(()=>{
  ...
  const partsNowET = fmt.formatToParts(now).reduce((acc: Record<string, string>, p) => { acc[p.type] = p.value; return acc; }, {});
  const todayMidnightETUtc = Date.UTC(Number(partsNowET.year), Number(partsNowET.month) - 1, Number(partsNowET.day), 0, 0, 0, 0);
  const tomorrowMidnightETUtc = todayMidnightETUtc + 24 * 60 * 60 * 1000;

  // Offset between ET and UTC for the current moment
  ...
  const nextMidnightET = tomorrowMidnightETUtc + offsetMs;
